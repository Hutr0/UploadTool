...
TEST_DESCRIPTION="Smoke: init.sh wizard (create .uploadtool + populate env.json/release.env + cli.env)"
...
assert_file_exists "$profile_path" "profile file should be created" || _failed=1

default_profile_file="${HOME}/.uploadtool/default_project"
assert_file_exists "$default_profile_file" "default_project marker should be created" || _failed=1
